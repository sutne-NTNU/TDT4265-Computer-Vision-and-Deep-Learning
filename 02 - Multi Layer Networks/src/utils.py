...
def set_default_plot_attributes():
...
def create_results_folder_if_not_exists():
    """ Creates a 'Results' folder to place all figures and images in to reduce clutter """
    if not os.path.exists('results'):
        os.makedirs('results')


def clear_file(filename: str):
    """ Clears the contents of a file, creates it if it doesn't exist"""
...
def write_markdown_header(filename: str):
    append_to_file(
        filename,
...
def write_results_to_file(filename: str, description: str, t_loss: float, v_loss: float, t_acc: float, v_acc: float):
    """ Writes the values to the file formatted correctly for markdown table """
    append_to_file(
        filename,
        f'|{description}|{t_loss:.4f}|{v_loss:.4f}|{100*t_acc:.3f}%|{100*v_acc:.3f}%|'
    )


def setup(results_file):
    """ Creates folders if they don't exist and resets the final results file. """
    create_results_folder_if_not_exists()
