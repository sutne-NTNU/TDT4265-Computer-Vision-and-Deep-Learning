...
def dataset_exists(dirpath: pathlib.Path, num_images):
    if not dirpath.is_dir():
        return False
    error_msg = (
        f"MNIST dataset already generated in {dirpath}, \n\tbut did not find filepath:"
    )
    error_msg2 = f"You can delete the directory by running: rm -r {dirpath.parent}"
    assert dirpath.joinpath(
        "images", "images.npy"
    ).is_file(), f"{error_msg}, {error_msg2}"
...
def load_dataset(
    ...
    for image_id in range(len(images)):
        label_path = dirpath.joinpath("labels").joinpath(f"{image_id}.txt")
