...
def download_dataset(zip_path, dataset_path, zip_url):
    ...
    if work_dir.is_dir():
        print(
            "You are working on a computer with the dataset under work_dataset:", work_dir)
        print("Doing nothing.")
        return
    if not zip_path.is_file():
        print(
            f"Download the zip file and place it in the path: {zip_path.absolute()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--old", default=False,
                        action="store_true", help="Download the old dataset")
