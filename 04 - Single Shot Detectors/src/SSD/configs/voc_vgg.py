...
train.epochs = math.ceil(5_000/256)
...
data_train.dataset = L(torch.utils.data.ConcatDataset)(datasets=[
    L(VOCDataset)(data_dir=get_dataset_dir("VOCdevkit/VOC2007"), split="train",
                  transform=train_cpu_transform, keep_difficult=True, remove_empty=True),
    L(VOCDataset)(data_dir=get_dataset_dir("VOCdevkit/VOC2012"), split="train",
                  transform=train_cpu_transform, keep_difficult=True, remove_empty=True)
])
...
label_map = {i: cls_name for i, cls_name in enumerate(VOCDataset.class_names)}
