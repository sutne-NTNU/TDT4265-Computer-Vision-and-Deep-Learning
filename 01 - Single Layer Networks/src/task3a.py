...
def cross_entropy_loss(targets: np.ndarray, outputs: np.ndarray):
    ...
    c_n = -np.sum(targets * np.log(outputs))
    loss = 1/outputs.shape[0] * np.sum(c_n)
    return loss
...
class SoftmaxModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        activation = np.dot(
            X,  # The Inputs (Pixel Values)
            self.w,  # The Weights from the inputs to our 10 outputs
        )
    ...
    for i in range(model.w.shape[0]):
        for j in range(model.w.shape[1]):
            ...
            assert abs(difference) <= epsilon**2,\
                ...
                actual gradient: \t{model.grad[i, j]}
                If this test fails there could be errors in your cross entropy loss function, forward function or backward function"""
